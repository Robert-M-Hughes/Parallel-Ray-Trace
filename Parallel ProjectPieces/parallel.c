...
/*Global Variables that all of the threads wil need access to*/
   	int num_threads = 0; // number of threads to be set by the command line
    double time_3 = 0;
...
void *threadWork(void* argstruct)
{
	...
	if(HEIGHT % num_threads != 0 && my_rank == num_threads - 1){//statement to fix the rows if the height is not divisible by 2^n
	int temp = HEIGHT - my_end;
	my_end = my_end + temp; // add the last portion to last thread so all pieces are covered
...
int main(int argc, char *argv[]){
	...
	lights[3].pos.y = 20;
	lights[3].pos.z = 100;
	lights[3].intensity.red = 5;
	lights[3].intensity.green = 5;
	lights[3].intensity.blue = 5;

    lights[4].pos.x = 300;
	lights[4].pos.y = 5300;
	lights[4].pos.z = -775;
	lights[4].intensity.red = 2;
	lights[4].intensity.green =2;
	lights[4].intensity.blue = 2;
...
printf("Main thread: All threads have been created.\n");
	...
	saveppm("imageParallel4lights.ppm", img, WIDTH, HEIGHT);
	printf("Drawing time: %f seconds\n", time_3 - time_1);
